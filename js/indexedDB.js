    ...
    // Função para Pesquisa no Banco de Dados com LIKE, Ordenação e Paginação
        ...
        function PesquisaRegistroBDIndex(bd, tabela, index, valor, ordenacao = "asc", paginacao = {}){
            return new Promise((resolve, reject)=>{
                ...
                var objBDStore = bd.result.transaction(tabela).objectStore(tabela);
                var objBDCursor = objBDStore.index(index).openCursor(null, ordenacao == "asc" ? "next" : "prev");
                var data = [];
    ...
    // Função para Pesquisa no Banco de Dados por Código
        ...
        function TotalRegistrosTabela(bd, tabela, pesquisa = {valor: "", index: ""}){
            return new Promise((resolve, reject)=>{
                if(typeof bd != "object"){
                    reject(console.error("O Objeto bd é inválido ou não existente"));
                }
                ...
                if(typeof pesquisa != "object"){
                    reject(console.error("O Objeto pesquisa é inválido ou não existente"));
                }
                
                var objBDStore = bd.result.transaction(tabela).objectStore(tabela);
                
                if(pesquisa.valor != ""){
                    //var objBDCursor = objBDStore.index(pesquisa.index).openCursor(IDBKeyRange.bound(pesquisa.valor, pesquisa.valor+"\uffff"), pesquisa.ordenacao == "asc" ? "next" : "prev");
                    var objBDCursor = objBDStore.index(pesquisa.index).openCursor(null, pesquisa.ordenacao == "asc" ? "next" : "prev");
                    var data = [];
    ...
    // Limpa Tabela
        ...
        function limpaTabelaBD(bd, tabela){
            if(typeof bd != "object"){
                reject(console.error("O Objeto bd é inválido ou não existente"));
            }
            ...
            bd.result.transaction(tabela, "readwrite").objectStore(tabela).clear();
        }
